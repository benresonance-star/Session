...
- **Session metadata:** optional **`description`** (multi-line, schema `maxLength` 2000) is documented in the JSON schema, edited in the builder (**session description** textarea), and shown on the **session detail** page when present. Sessions may also include an optional session-level external **`link`** (`{ url, label? }`) for a playlist, blog post, or video reference. On **link-only** sessions (no meaningful exercises beyond untouched draft scaffolding), the **session detail** page shows the external link instead of the empty stage / section scaffold and hides the **start** action. For YouTube links, the detail page prefers a **bookmark-style preview card** (thumbnail + title / provider metadata when available) rather than a bare hostname or icon-only link, with graceful fallback when preview metadata cannot be fetched.
...
- optional session-level **`link`** (`url` + optional `label`) for an external reference such as a YouTube video or article
...
- when a session has a session-level **`link`**, show it on the detail page; if the session is **link-only** (no meaningful exercises beyond untouched builder placeholders), suppress the empty stage / section preview and hide the **start** action
- for YouTube session links on the detail page, prefer a **bookmark-style preview** with thumbnail and metadata (similar to a Notion bookmark card) instead of a plain text hostname or icon-only link
...
- start session action (hidden for link-only sessions)
